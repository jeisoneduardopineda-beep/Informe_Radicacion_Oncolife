...
def month_chart(items: list[dict]) -> str:
    ...
    for item in items:
        ...
        cols.append(
            f"""
            <div class="month-col">
              <div class="month-value">{money(item['value'])}</div>
              <div class="month-bar" style="height:{height:.0f}px"></div>
              <div class="month-label">{item['label'][:3]}</div>
            </div>
            """
        )
...
def admin_panel() -> None:
    ...
    admin_password = st.secrets.get("ADMIN_PASSWORD", "cambiar-esta-clave")
    ...
    with tab1:
        ...
        with right:
            ...
            for idx, item in enumerate(summary["top_convenios"][:5], start=1):
                rank.append(f"<div class='rank-row'><b>{idx}</b><span>{item['label']}</span><strong>{money(item['value'])}</strong></div>")
