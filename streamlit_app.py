...
def css() -> None:
    st.markdown(
        ...
        <style>
        :root {
          --bg:#0b1f2a; --panel:#14323b; --panel2:#102a34; --line:#3d626b;
          --ink:#f4fbfa; --muted:#bfd0d0; --blue:#9ee4de; --amber:#ffd48a; --rose:#f5b6c5;
        }
        .stApp { background:#0b1f2a; color:var(--ink); }
        [data-testid="stSidebar"] { background:#102a34; border-right:1px solid var(--line); }
        h1, h2, h3 { color:#edf7f8; letter-spacing:0; }
        .muted { color:var(--muted); font-size:13px; }
        .kpi {
          background:linear-gradient(135deg,#1d4a52,#16313b);
          border:1px solid #3d626b;
          padding:16px;
          min-height:105px;
        }
        ...
        .kpi em { display:block; color:#ffd48a; font-style:normal; font-weight:700; margin-top:6px; }
        .panel {
          background:#14323b;
          border:1px solid #3d626b;
          padding:14px;
        ...
        .bar-track { height:17px; background:#284852; border:1px solid #55727a; }
        .bar-fill { height:100%; background:linear-gradient(90deg,#ffd48a,#f5b6c5,#9ee4de); }
        ...
        .rank-row b {
          ...
          background:#ffd48a; color:#061823;
        }
        ...
        .month-chart {
          ...
          align-items:end;
          gap:10px;
          border-bottom:1px solid #3d626b;
          padding:18px 8px 0;
        }
        ...
        .month-bar { width:72%; min-width:24px; background:linear-gradient(180deg,#ffd48a,#f5b6c5,#9ee4de); }
...
def available_sources() -> list[tuple[str, Path, float]]:
    candidates = []
    if LATEST_FILE.exists():
        candidates.append(("radicacion", LATEST_FILE, LATEST_FILE.stat().st_mtime))
    if LATEST_RIPS_FILE.exists():
...
def get_data() -> tuple[str, pd.DataFrame, dict] | None:
    ...
    if not sources:
        ...
        return None
    errors = []
    for kind, path, mtime in sources:
        try:
            if kind == "rips":
                df, metadata = load_rips_from_disk(str(path), mtime)
                st.session_state.pop("load_error", None)
                return kind, df, metadata
            df = load_from_disk(str(path), mtime)
            st.session_state.pop("load_error", None)
            return kind, df, {}
        except Exception as exc:
            errors.append(f"{path.name}: {exc}")
    st.session_state["load_error"] = " | ".join(errors)
    return None


def save_uploaded(uploaded_file) -> tuple[pd.DataFrame, bool]:
    signature = f"{uploaded_file.name}:{getattr(uploaded_file, 'size', '')}"
    if (
        st.session_state.get("last_upload_signature") == signature
        and "df" in st.session_state
        and "data_kind" in st.session_state
    ):
        return st.session_state["df"], False

    DATA_DIR.mkdir(parents=True, exist_ok=True)
    payload = uploaded_file.getvalue()
    content_hash = hashlib.sha256(payload).hexdigest()
    signature = f"{uploaded_file.name}:{len(payload)}:{content_hash}"
    if (
        st.session_state.get("last_upload_signature") == signature
        and "df" in st.session_state
        and "data_kind" in st.session_state
    ...
    for key in [
        "radicacion_pdf_bytes",
        "radicacion_excel_bytes",
        "rips_pdf_bytes",
        "rips_excel_bytes",
    ]:
...
def rips_group_users(df: pd.DataFrame, field: str, top: int | None = None) -> list[dict]:
    users = df[["Documento Usuario", field]].drop_duplicates()
    grouped = users.groupby(field, dropna=False)["Documento Usuario"].nunique().sort_values(ascending=False).reset_index(name="Cantidad")
    if top:
        grouped = grouped.head(top)
    return [{"label": str(row[field]) or "Sin dato", "value": int(row["Cantidad"])} for _, row in grouped.iterrows()]
...
def rips_month_summary(df: pd.DataFrame) -> list[dict]:
    ...
    if valid.empty:
        return []
    valid["Dia"] = valid["Fecha Servicio"].dt.date.astype(str)
    grouped = valid.groupby("Dia").size().reset_index(name="Cantidad")
    return [{"label": row["Dia"], "value": int(row["Cantidad"]), "count": int(row["Cantidad"])} for _, row in grouped.iterrows()]


def rips_top_by_service_type(df: pd.DataFrame, service_type: str, top: int = 10) -> list[dict]:
    piece = df[df["Tipo Servicio"] == service_type].copy()
    if piece.empty:
        return []
    piece["Item Servicio"] = piece["Nombre Tecnologia"].where(piece["Nombre Tecnologia"] != "", piece["Codigo Servicio"])
    piece["Item Servicio"] = piece["Item Servicio"].where(piece["Item Servicio"] != "", "Sin codigo")
    grouped = piece.groupby("Item Servicio").size().sort_values(ascending=False).reset_index(name="Cantidad").head(top)
    return [{"label": str(row["Item Servicio"]), "value": int(row["Cantidad"])} for _, row in grouped.iterrows()]
...
def build_rips_summary(df: pd.DataFrame, metadata: dict) -> dict:
    ...
    return {
        ...
        "sexo": rips_group_users(df, "Sexo"),
        "tipo_usuario": rips_group_users(df, "Tipo Usuario"),
        "top_consultas": rips_top_by_service_type(df, "consultas", 10),
        "top_procedimientos": rips_top_by_service_type(df, "procedimientos", 10),
        "top_medicamentos": rips_top_by_service_type(df, "medicamentos", 10),
        "top_otros_servicios": rips_top_by_service_type(df, "otrosServicios", 10),
    }
...
def pdf_table(title: str, rows: list[list], widths: list[float] | None = None) -> list:
    ...
    table.setStyle(
        TableStyle(
            [
                ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#D8F3EF")),
                ("TEXTCOLOR", (0, 0), (-1, 0), colors.HexColor("#0B1F2A")),
                ("FONTNAME", (0, 0), (-1, 0), "Helvetica-Bold"),
                ("FONTSIZE", (0, 0), (-1, 0), 8.5),
                ("FONTSIZE", (0, 1), (-1, -1), 7.5),
                ("GRID", (0, 0), (-1, -1), 0.25, colors.HexColor("#CDE4E6")),
                ("ROWBACKGROUNDS", (0, 1), (-1, -1), [colors.white, colors.HexColor("#FFF5E1")]),
                ("VALIGN", (0, 0), (-1, -1), "TOP"),
                ("ALIGN", (-1, 1), (-1, -1), "RIGHT"),
...
def make_rips_excel_report(df: pd.DataFrame, summary: dict) -> bytes:
    ...
    cards = [
        ...
        ("J6:M9", "Diagnosticos", k["diagnosticos"], "#,##0", rose),
    ]
    for rng, label, value, fmt, color in cards:
        ...
        cell.value = value
        cell.number_format = fmt
        cell.font = Font(size=18, bold=True, color="071823" if color in [amber, teal, rose] else "FFFFFF")
        cell.fill = PatternFill("solid", fgColor=color)
        cell.alignment = Alignment(horizontal="center", vertical="center")
        cell.border = Border(left=line, right=line, top=line, bottom=line)
    ...
    write_count_table(26, 1, "Top 10 consultas", summary["top_consultas"], "Consulta")
    write_count_table(26, 4, "Top 10 procedimientos", summary["top_procedimientos"], "Procedimiento")
    write_count_table(26, 8, "Top 10 medicamentos", summary["top_medicamentos"], "Medicamento")
    write_count_table(26, 11, "Top 10 otros servicios", summary["top_otros_servicios"], "Otro servicio")
...
def make_rips_pdf_report(df: pd.DataFrame, summary: dict, filters: dict[str, str]) -> bytes:
    ...
    hero.setStyle(TableStyle([("BACKGROUND", (0, 0), (-1, -1), colors.HexColor("#0B1F2A")), ("LEFTPADDING", (0, 0), (-1, -1), 16)]))
    ...
    kpi_table.setStyle(
        TableStyle(
            [
                ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#14323B")),
                ("BACKGROUND", (0, 1), (-1, 1), colors.HexColor("#1D4A52")),
                ("TEXTCOLOR", (0, 0), (-1, -1), colors.white),
                ("FONTNAME", (0, 0), (-1, -1), "Helvetica-Bold"),
                ("FONTSIZE", (0, 0), (-1, 0), 8),
                ("FONTSIZE", (0, 1), (-1, 1), 14),
                ("ALIGN", (0, 0), (-1, -1), "CENTER"),
                ("GRID", (0, 0), (-1, -1), 0.6, colors.HexColor("#9EE4DE")),
            ]
        )
    )
    story.extend([kpi_table, Spacer(1, 0.18 * inch)])
    ...
    story.extend(pdf_table("Top 10 consultas", [["Consulta", "Cantidad"]] + [[i["label"][:70], number(i["value"])] for i in summary["top_consultas"]], [5.0 * inch, 1.2 * inch]))
    story.extend(pdf_table("Top 10 procedimientos", [["Procedimiento", "Cantidad"]] + [[i["label"][:70], number(i["value"])] for i in summary["top_procedimientos"]], [5.0 * inch, 1.2 * inch]))
    story.extend(pdf_table("Top 10 medicamentos", [["Medicamento", "Cantidad"]] + [[i["label"][:70], number(i["value"])] for i in summary["top_medicamentos"]], [5.0 * inch, 1.2 * inch]))
    story.extend(pdf_table("Top 10 otros servicios", [["Otro servicio", "Cantidad"]] + [[i["label"][:70], number(i["value"])] for i in summary["top_otros_servicios"]], [5.0 * inch, 1.2 * inch]))
...
def admin_panel() -> None:
    ...
    if st.sidebar.button("Borrar datos guardados"):
        for path in [LATEST_FILE, LATEST_RIPS_FILE]:
            if path.exists():
                path.unlink()
        for key in ["df", "data_kind", "metadata", "load_error", "last_upload_signature"]:
            st.session_state.pop(key, None)
        load_from_disk.clear()
        load_rips_from_disk.clear()
        st.rerun()
    ...
    if uploaded:
        try:
            df, changed = save_uploaded(uploaded)
...
def render_dashboard(df_raw: pd.DataFrame) -> None:
    ...
    report_key = json.dumps(filters, sort_keys=True) + f":{len(df)}:{float(df['Saldo Apb'].sum()):.2f}"
    d1, d2 = st.columns([1, 1])
    with d1:
        if st.button("Generar PDF", key="generate_radicacion_pdf"):
            st.session_state["radicacion_pdf_bytes"] = make_pdf_report(df, summary, filters)
            st.session_state["radicacion_pdf_key"] = report_key
        if st.session_state.get("radicacion_pdf_key") == report_key and "radicacion_pdf_bytes" in st.session_state:
            st.download_button("Descargar PDF", st.session_state["radicacion_pdf_bytes"], "reporte_radicacion.pdf", "application/pdf")
    with d2:
        if st.button("Generar Excel", key="generate_radicacion_excel"):
            st.session_state["radicacion_excel_bytes"] = make_excel_report(df, summary)
            st.session_state["radicacion_excel_key"] = report_key
        if st.session_state.get("radicacion_excel_key") == report_key and "radicacion_excel_bytes" in st.session_state:
            st.download_button("Descargar Excel", st.session_state["radicacion_excel_bytes"], "reporte_radicacion.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
...
def render_rips_dashboard(df_raw: pd.DataFrame, metadata: dict) -> None:
    ...
    report_key = json.dumps(filters, sort_keys=True) + f":{len(df)}:{float(df['Vr Servicio'].sum()):.2f}"
    d1, d2 = st.columns([1, 1])
    with d1:
        if st.button("Generar PDF RIPS", key="generate_rips_pdf"):
            st.session_state["rips_pdf_bytes"] = make_rips_pdf_report(df, summary, filters)
            st.session_state["rips_pdf_key"] = report_key
        if st.session_state.get("rips_pdf_key") == report_key and "rips_pdf_bytes" in st.session_state:
            st.download_button("Descargar PDF RIPS", st.session_state["rips_pdf_bytes"], "reporte_rips.pdf", "application/pdf")
    with d2:
        if st.button("Generar Excel RIPS", key="generate_rips_excel"):
            st.session_state["rips_excel_bytes"] = make_rips_excel_report(df, summary)
            st.session_state["rips_excel_key"] = report_key
        if st.session_state.get("rips_excel_key") == report_key and "rips_excel_bytes" in st.session_state:
            st.download_button("Descargar Excel RIPS", st.session_state["rips_excel_bytes"], "reporte_rips.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

    tab1, tab2, tab3, tab4, tab5 = st.tabs(["Resumen", "Top servicios", "Diagnosticos", "Tecnologias", "Detalle"])
    with tab1:
        left, right = st.columns([2, 1])
        with left:
    ...
    with tab2:
        a, b = st.columns(2)
        with a:
            st.markdown("<div class='panel'><h3>Top 10 consultas</h3>" + bars(summary["top_consultas"], 10, "count") + "</div>", unsafe_allow_html=True)
            st.markdown("<br>", unsafe_allow_html=True)
            st.markdown("<div class='panel'><h3>Top 10 medicamentos</h3>" + bars(summary["top_medicamentos"], 10, "count") + "</div>", unsafe_allow_html=True)
        with b:
            st.markdown("<div class='panel'><h3>Top 10 procedimientos</h3>" + bars(summary["top_procedimientos"], 10, "count") + "</div>", unsafe_allow_html=True)
            st.markdown("<br>", unsafe_allow_html=True)
            st.markdown("<div class='panel'><h3>Top 10 otros servicios</h3>" + bars(summary["top_otros_servicios"], 10, "count") + "</div>", unsafe_allow_html=True)
    with tab3:
...
def main() -> None:
    ...
    if loaded is None:
        ...
        if st.session_state.get("load_error"):
            st.error("No pude leer el archivo guardado. Carga nuevamente un Excel con hoja BASE o un JSON RIPS valido.")
